...
export async function addToInventory(req: Request, res: Response) {
  ...
  const { classId, itemId, quantity } = req.body as { classId: string, itemId: string, quantity: number };
...
export async function removeFromInventory(req: Request, res: Response) {
  ...
  try {
    ...
    if (deletedItem) {
      return res.status(200).json({ message: "Item successfully removed from inventory" });
    } else {
      return res.status(404).json({ message: "Item not found in inventory" });
    }
  } catch (error) {
    return res.status(500).json({ message: "Error removing item from inventory", error: error.message });
  }
}
